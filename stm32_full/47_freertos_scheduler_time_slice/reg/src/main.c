...
static void same_a(void *arg){ (void)arg; while(1){ led_toggle_pa1(); taskYIELD(); } }
static void same_b(void *arg){ (void)arg; while(1){ led_toggle_pa2(); taskYIELD(); } }
static void high_task(void *arg){ (void)arg; while(1){ led_toggle_pc13(); vTaskDelay(pdMS_TO_TICKS(500)); } }
int main(void){ system_clock_72mhz_init(); gpio_init(); BaseType_t ok=xTaskCreate(same_a,"a",128,NULL,1,NULL); ok &= xTaskCreate(same_b,"b",128,NULL,1,NULL); ok &= xTaskCreate(high_task,"high",128,NULL,2,NULL); if(ok!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
