...
static void taker(void *arg){ (void)arg; while(1){ if(xSemaphoreTake(g_bin,portMAX_DELAY)==pdTRUE){ led_toggle_pc13(); } } }
static void worker(void *arg){ (void)arg; while(1){ if(xSemaphoreTake(g_count,portMAX_DELAY)==pdTRUE){ led_toggle_pa1(); vTaskDelay(pdMS_TO_TICKS(300)); xSemaphoreGive(g_count); } } }
int main(void){ system_clock_72mhz_init(); gpio_init(); g_bin=xSemaphoreCreateBinary(); g_count=xSemaphoreCreateCounting(2,2); BaseType_t ok=xTaskCreate(giver,"give",128,NULL,2,NULL); ok &= xTaskCreate(taker,"take",128,NULL,1,NULL); ok &= xTaskCreate(worker,"work",128,NULL,1,NULL); if(g_bin==NULL||g_count==NULL||ok!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
