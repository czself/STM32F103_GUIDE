...
static void led_task(void *arg){ (void)arg; while(1){ HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); vTaskDelay(pdMS_TO_TICKS(500)); } }
static void heartbeat_task(void *arg){ (void)arg; while(1){ HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_1); vTaskDelay(pdMS_TO_TICKS(1000)); } }
int main(void){ HAL_Init(); system_clock_72mhz_init(); gpio_init(); BaseType_t ok=xTaskCreate(led_task,"led",128,NULL,2,NULL); ok &= xTaskCreate(heartbeat_task,"beat",128,NULL,1,NULL); if(ok!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
