...
本课代码随后 `HAL_Delay(200)`，给 DMA 和 USART 留出发送时间，再翻转 LED。因为字符串很短，115200 波特率下 16 字节发送时间约几毫秒量级，200ms 足够。
...
进入 `19_dma_memory_uart_cases/reg`，重点读 `app_init()`、`dma_mem_copy()`、`dma_uart_send()`。确认 GPIOA/USART1/DMA1 时钟打开，PA9 是复用推挽，USART1 打开 `DMAT/TE/UE`，Channel1 使用 `MEM2MEM`，Channel4 使用 `DIR/MINC`。
...
进入 `19_dma_memory_uart_cases/hal`，重点读 `uart_dma_init()` 和主循环。确认 `huart1` 是 USART1 TX，`hdma_tx` 是 DMA1_Channel4，方向是 `DMA_MEMORY_TO_PERIPH`，并且执行了 `__HAL_LINKDMA(&huart1, hdmatx, hdma_tx)`。
...
最常见的是 PA9/RX 接反、忘记共地、USART 波特率和系统时钟不匹配、忘记 `USART_CR3_DMAT`、DMA 通道选错、USART DMA 误开 `PINC`、HAL 版忘记 `__HAL_LINKDMA()`。这些错误的现象不同，要按路径排查。
...
HAL 版还要检查 `HAL_DMA_Init(&hdma_tx)` 是否成功，`hdma_tx.Instance` 是否为 `DMA1_Channel4`，以及 `__HAL_LINKDMA(&huart1, hdmatx, hdma_tx)` 是否在发送前执行。
...
6. HAL 版 UART DMA 的关键不是只调用发送函数，而是先把 UART 句柄和 TX DMA 句柄通过 `hdmatx` 关联。
