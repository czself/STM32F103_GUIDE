...
static void precise_task(void *arg){ TickType_t last=xTaskGetTickCount(); (void)arg; while(1){ HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); vTaskDelayUntil(&last,pdMS_TO_TICKS(500)); } }
static void relative_task(void *arg){ (void)arg; while(1){ HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_1); vTaskDelay(pdMS_TO_TICKS(700)); } }
int main(void){ HAL_Init(); system_clock_72mhz_init(); gpio_init(); BaseType_t ok=xTaskCreate(precise_task,"until",128,NULL,2,NULL); ok &= xTaskCreate(relative_task,"delay",128,NULL,1,NULL); if(ok!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
