...
static void memory_task(void *arg){ (void)arg; while(1){ g_stack_watermark=uxTaskGetStackHighWaterMark(NULL); HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); vTaskDelay(pdMS_TO_TICKS(500)); } }
int main(void){ HAL_Init(); system_clock_72mhz_init(); gpio_init(); if(xTaskCreate(memory_task,"mem",160,NULL,1,NULL)!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
