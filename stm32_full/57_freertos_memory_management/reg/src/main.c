...
static void memory_task(void *arg){ (void)arg; while(1){ g_stack_watermark=uxTaskGetStackHighWaterMark(NULL); led_toggle_pc13(); vTaskDelay(pdMS_TO_TICKS(500)); } }
int main(void){ system_clock_72mhz_init(); gpio_init(); if(xTaskCreate(memory_task,"mem",160,NULL,1,NULL)!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
