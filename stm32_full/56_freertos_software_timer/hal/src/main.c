...
static void timer_cb(TimerHandle_t timer){ (void)timer; HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); }
static void monitor(void *arg){ (void)arg; while(1){ HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_1); vTaskDelay(pdMS_TO_TICKS(1000)); } }
int main(void){ HAL_Init(); system_clock_72mhz_init(); gpio_init(); g_timer=xTimerCreate("blink",pdMS_TO_TICKS(500),pdTRUE,NULL,timer_cb); BaseType_t ok=xTaskCreate(monitor,"mon",128,NULL,1,NULL); if(g_timer==NULL||ok!=pdPASS||xTimerStart(g_timer,0)!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
