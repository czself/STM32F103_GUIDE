...
static void low_task(void *arg){ (void)arg; while(1){ xSemaphoreTake(g_mutex,portMAX_DELAY); HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_1); vTaskDelay(pdMS_TO_TICKS(600)); xSemaphoreGive(g_mutex); vTaskDelay(pdMS_TO_TICKS(300)); } }
static void mid_task(void *arg){ (void)arg; while(1){ HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_2); vTaskDelay(pdMS_TO_TICKS(100)); } }
static void high_task(void *arg){ (void)arg; while(1){ vTaskDelay(pdMS_TO_TICKS(200)); xSemaphoreTake(g_mutex,portMAX_DELAY); HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); xSemaphoreGive(g_mutex); } }
int main(void){ HAL_Init(); system_clock_72mhz_init(); gpio_init(); g_mutex=xSemaphoreCreateMutex(); BaseType_t ok=xTaskCreate(low_task,"low",128,NULL,1,NULL); ok &= xTaskCreate(mid_task,"mid",128,NULL,2,NULL); ok &= xTaskCreate(high_task,"high",128,NULL,3,NULL); if(g_mutex==NULL||ok!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
