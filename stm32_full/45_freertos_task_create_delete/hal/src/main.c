...
static void worker_task(void *arg){ (void)arg; for(uint8_t i=0;i<6;i++){ HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); vTaskDelay(pdMS_TO_TICKS(200)); } g_worker=NULL; vTaskDelete(NULL); }
static void creator_task(void *arg){ (void)arg; while(1){ if(g_worker==NULL){ (void)xTaskCreate(worker_task,"work",128,NULL,2,&g_worker); } vTaskDelay(pdMS_TO_TICKS(2000)); } }
int main(void){ HAL_Init(); system_clock_72mhz_init(); gpio_init(); if(xTaskCreate(creator_task,"make",160,NULL,1,NULL)!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
