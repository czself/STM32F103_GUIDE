...
static void worker_task(void *arg){ (void)arg; for(uint8_t i=0;i<6;i++){ led_toggle_pc13(); vTaskDelay(pdMS_TO_TICKS(200)); } g_worker=NULL; vTaskDelete(NULL); }
...
int main(void){ system_clock_72mhz_init(); gpio_init(); if(xTaskCreate(creator_task,"make",160,NULL,1,NULL)!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
