...
static void slow_task(void *arg){ (void)arg; while(1){ led_toggle_pc13(); vTaskDelay(pdMS_TO_TICKS(2000)); } }
int main(void){ system_clock_72mhz_init(); gpio_init(); if(xTaskCreate(slow_task,"slow",128,NULL,1,NULL)!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
