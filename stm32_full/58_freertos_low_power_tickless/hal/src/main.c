...
void vApplicationSleep(TickType_t expected_idle_time){ (void)expected_idle_time; __WFI(); }
static void slow_task(void *arg){ (void)arg; while(1){ HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); vTaskDelay(pdMS_TO_TICKS(2000)); } }
int main(void){ HAL_Init(); system_clock_72mhz_init(); gpio_init(); if(xTaskCreate(slow_task,"slow",128,NULL,1,NULL)!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
