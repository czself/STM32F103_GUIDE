...
static void task_a(void *arg){ (void)arg; while(1){ xEventGroupSetBits(g_events,BIT_A); HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_1); vTaskDelay(pdMS_TO_TICKS(500)); } }
static void task_b(void *arg){ (void)arg; while(1){ xEventGroupSetBits(g_events,BIT_B); HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_2); vTaskDelay(pdMS_TO_TICKS(900)); } }
static void wait_task(void *arg){ (void)arg; while(1){ xEventGroupWaitBits(g_events,BIT_A|BIT_B,pdTRUE,pdTRUE,portMAX_DELAY); HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); } }
int main(void){ HAL_Init(); system_clock_72mhz_init(); gpio_init(); g_events=xEventGroupCreate(); BaseType_t ok=xTaskCreate(task_a,"a",128,NULL,1,NULL); ok &= xTaskCreate(task_b,"b",128,NULL,1,NULL); ok &= xTaskCreate(wait_task,"wait",160,NULL,2,NULL); if(g_events==NULL||ok!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
