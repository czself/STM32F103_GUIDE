...
static void consumer(void *arg){ uint8_t e; (void)arg; while(1){ if(xQueueReceive(g_queue,&e,portMAX_DELAY)==pdPASS){ led_toggle_pc13(); } } }
int main(void){ system_clock_72mhz_init(); gpio_init(); g_queue=xQueueCreate(4,sizeof(uint8_t)); BaseType_t ok=xTaskCreate(producer,"prod",128,NULL,2,NULL); ok &= xTaskCreate(consumer,"cons",128,NULL,1,NULL); if(g_queue==NULL||ok!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
