...
static void producer(void *arg){ uint8_t e=1; (void)arg; while(1){ xQueueSend(g_queue,&e,0); vTaskDelay(pdMS_TO_TICKS(700)); } }
static void consumer(void *arg){ uint8_t e; (void)arg; while(1){ if(xQueueReceive(g_queue,&e,portMAX_DELAY)==pdPASS){ HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); } } }
int main(void){ HAL_Init(); system_clock_72mhz_init(); gpio_init(); g_queue=xQueueCreate(4,sizeof(uint8_t)); BaseType_t ok=xTaskCreate(producer,"prod",128,NULL,2,NULL); ok &= xTaskCreate(consumer,"cons",128,NULL,1,NULL); if(g_queue==NULL||ok!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
