...
static void sender(void *arg){ (void)arg; while(1){ vTaskDelay(pdMS_TO_TICKS(500)); xTaskNotifyGive(g_led_task); } }
static void led_task(void *arg){ (void)arg; while(1){ ulTaskNotifyTake(pdTRUE,portMAX_DELAY); HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); } }
int main(void){ HAL_Init(); system_clock_72mhz_init(); gpio_init(); BaseType_t ok=xTaskCreate(led_task,"led",128,NULL,2,&g_led_task); ok &= xTaskCreate(sender,"send",128,NULL,1,NULL); if(ok!=pdPASS){taskDISABLE_INTERRUPTS();while(1){}} vTaskStartScheduler(); while(1){} }
